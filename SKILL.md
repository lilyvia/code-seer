...
### Bash 超时配置
**所有 bash 命令调用必须使用 10 分钟（600000 毫秒）超时**，避免大型项目扫描中断：

```bash
# 正确示例 - 带 10 分钟超时
bash(command="python3 scripts/security_scanner.py /path/to/project", timeout=600000)

# 默认超时时间
DEFAULT_TIMEOUT = 600000  # 10 分钟 = 600 秒 = 600000 毫秒
```
...
- 结合规则库、按需执行 python3、ast-grep、grep 来获取佐证证据，但结论应来自综合分析结果，而非单次脚本输出。
