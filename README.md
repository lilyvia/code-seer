...
OpenCode 安全审计技能，基于 AST-grep 规则库进行自动化安全漏洞检测与分析，当前提供 79 条规则，支持 8 种编程语言（其中 JavaScript 规则承担 JavaScript/TypeScript 项目的 Wave 1 首轮扫描），覆盖 14 类常见漏洞类型。
...
- **PHP** (`.php`) - PHP常见漏洞模式
- **C#** (`.cs`) - .NET平台安全检测
- **JavaScript/TypeScript** (`.js`/`.ts`) - Wave 1 统一由 JavaScript 规则执行首轮扫描，适用于前端与 Node.js 代码库；当前仓库未单独维护 TypeScript 规则矩阵
- **Ruby** (`.rb`) - Rails/Sinatra 安全检测
- **Rust** (`.rs`) - Rust 安全检测
...
│   │   ├── ssti-*.yml         # SSTI规则
│   │   ├── nosql-injection-*.yml # NoSQL注入规则
│   │   ├── prototype-pollution-*.yml # 原型污染规则
│   │   ├── jndi-injection-*.yml    # JNDI注入规则
│   │   └── open-redirect-*.yml     # 开放重定向规则
